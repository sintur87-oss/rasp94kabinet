# -*- coding: utf-8 -*-
"""
Rasp Bot v2.2 (constants-based configuration)
---------------------------------------------
Эта версия повторяет логику оригинального бота расписания, но вместо
использования переменных окружения использует модульные константы.
Все ключи и настройки сосредоточены в одном месте в начале файла.
"""
...
# Укажите актуальный токен бота перед запуском.
TOKEN = ""
YANDEX_OAUTH_TOKEN = ""
YANDEX_API_URL = "https://cloud-api.yandex.net/v1/disk/resources"
YANDEX_FOLDER_PATH = "rasp"
ADMIN_ID = 0

QUIET_START_HOUR = 23
QUIET_END_HOUR = 7
HOLIDAYS_START = "27.10.2025"
HOLIDAYS_END = "04.11.2025"
...
# ---------- Локаль ----------
for locale_name in ("ru_RU", "ru_RU.UTF-8"):
    try:
...
logger = logging.getLogger("rasp-bot-v2.2-constants")
...
def get_today_date_str(offset: int = 0) -> str:
    ...
    if today.weekday() == 6:  # воскресенье пропускаем
...
async def handle_text_query(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if date_str:
        await display_final_schedule(update, date_str, class_name, context)
    else:
